...
# =============================================================================
    ...
    def subtractionCheck(self, image, ref, diff, inset_size=21,
                         expected_location=[], 
                         fitted_location=[], 
                         mask=None, 
                         aperture_size=7,
        ...
        try:
            ...
            def get_inset_side(val, size, axis_len):
                if val > axis_len * (1 - margin):
                    return 'low', val - size, 0.0
                elif val < axis_len * margin:
                    return 'high', val + size, 1.0
                else:
                    side = 'high' if val < axis_len / 2 else 'low'
                    main = val + size if side == 'high' else val - size
            ...
            for i, (title, img_data) in enumerate(images.items()):
                ...
                ax.imshow(img_data, origin='lower', aspect='auto', cmap='bone', 
                          vmin=vmins[title], vmax=vmaxs[title])
                ax.set_title(title)
            ...
            if matching_sources is not None and len(matching_sources) > 0:
                ...
                for ax in axes:  # Only first two panels (Image and Reference)
                    for x_pix, y_pix in zip(matching_sources['x_pix'], matching_sources['y_pix']):
                        x_pix -=1
                        y_pix -=1
                        rect = patches.Rectangle(
                            (x_pix - half_size, y_pix - half_size),
                            square_size, square_size,
                            linewidth=0.5, edgecolor='b', facecolor='none', alpha=0.5
                        )
                        ax.add_patch(rect)
            ...
            if masked_sources is not None and len(masked_sources) > 0:
                ...
                for x, y, otype, name in zip(masked_sources['x_pix'], masked_sources['y_pix'], masked_sources['OTYPE_opt'], masked_sources['MAIN_ID']):
                    x -=1
                    y -=1
                    if 'SN*' in otype:
                        otype = name
                        circle = mpatches.Circle((x, y), cross_len * 2, edgecolor='red', facecolor='none', zorder=4, lw=0.5)
                        axes[0].add_patch(circle)  # Assuming add to first ax, adjust if needed
                    else:
                        axes[0].plot([x - cross_len, x + cross_len], [y - cross_len, y + cross_len], color='red', lw=0.5, zorder=2)
                        axes[0].plot([x - cross_len, x + cross_len], [y + cross_len, y - cross_len], color='red', lw=0.5, zorder=2)
                    axes[0].annotate(
                        otype,
                        xy=(x, y),
            ...
            for i, (title, img_data) in enumerate(images.items()):
                ...
                if expected_location:
                    ...
                    x_side, con_x_main, con_x_inset = get_inset_side(x, inset_size, img_width)
                    y_side, con_y_main, con_y_inset = get_inset_side(y, inset_size, img_height)
                    ...
                    ax_inset.imshow(img_data, origin='lower', aspect='auto', cmap='bone', 
                                    vmin=vmins[title], vmax=vmaxs[title])
                    ...
                    rect = Rectangle((x - inset_size, y - inset_size), 2 * inset_size, 2 * inset_size,
                                     linewidth=0.5, edgecolor='red', facecolor='none')
                    ax.add_patch(rect)
    
                    # Connection lines
                    if inset_loc == 'upper left':
                        corners = [((x - inset_size, y - inset_size), (0, 0)), ((x + inset_size, y + inset_size), (1, 1))]
                    elif inset_loc == 'upper right':
                        corners = [((x + inset_size, y - inset_size), (1, 0)), ((x - inset_size, y + inset_size), (0, 1))]
                    ...
                    for (main_pt, inset_pt) in corners:
                        fig.add_artist(ConnectionPatch(
                            ...
                            xyB=inset_pt, coordsB=ax_inset.transAxes,
            ...
            if fitted_location and len(fitted_location) == 2:
                ...
                for ax in inset_axes_list[2:]:
                    ...
                    circle = mpatches.Circle(
                        fitted_location, radius=radius,
                        edgecolor='red', facecolor='none', linewidth=0.5, transform=ax.transData
                    )
                    ax.add_patch(circle)
                    ...
                    hline = mlines.Line2D(
                        [x - cross_len, x + cross_len], [y, y],
                        color='green', linewidth=0.5, transform=ax.transData
                    )
                    vline = mlines.Line2D(
                        [x, x], [y - cross_len, y + cross_len],
                        color='green', linewidth=0.5, transform=ax.transData
                    )
                    ax.add_line(hline)
                    ax.add_line(vline)
            ...
            logger.error(
                "subtractionCheck failed: %s in %s:%d",
                exc_type.__name__,
                fname,
                line,
                exc_info=True,
...
# =============================================================================
    ...
    def crowding_target(
        ...
        fig, axes = plt.subplots(1, 3, figsize=set_size(540, 1), constrained_layout=True)
        ...
        if aperture_radius is not None and np.isfinite(aperture_radius) and aperture_radius > 0:
            axes[0].add_patch(
                mpatches.Circle((tx, ty), float(aperture_radius), edgecolor="cyan", facecolor="none", lw=0.8)
            )
        ...
        if levels.size:
            axes[1].contour(seg, levels=levels, colors="lime", linewidths=0.4, alpha=0.9)
    ...
    def SourceCheck(self, image,
        ...
        try:
            ...
            im = ax1.imshow(image, origin='lower', aspect='auto', cmap='grey_r', interpolation=None, norm=norm)
            ...
            circle = Circle(
                ...
                radius, edgecolor='#D55E00', facecolor='none', zorder=4, lw=0.5
            )
            ax1.add_patch(circle)
            ax1.text(
                self.input_yaml['target_x_pix'], self.input_yaml['target_y_pix'] + radius + 2,
                self.input_yaml['target_name'], color='#D55E00', fontsize=3, ha='center', va='bottom'
            )
            ...
            if catalogSources is not None and not self.input_yaml['HST_mode']:
                for x, y in zip(catalogSources['x_pix'], catalogSources['y_pix']):
                    ...
                    square = Rectangle(
                        ...
                        edgecolor='#0072B2', facecolor='none',
                        label='Reference Sources', zorder=1, lw=0.5
                    )
            ...
            if FWHMSources is not None and 'fwhm' in FWHMSources:
                ...
                if len(FWHMSources)>0:
                    ...
                    norm_fwhm = Normalize(vmin=np.nanmin(fwhm_values), vmax=np.nanmax(fwhm_values))
                    ...
                    for x, y, fwhm in zip(FWHMSources['x_pix'], FWHMSources['y_pix'], fwhm_values):
                        if np.isnan(fwhm): continue
                        color = cmap(norm_fwhm(fwhm))
                        circle = Circle(
                            (x, y), fwhm, edgecolor=color, facecolor='none', 
                            label='FWHM Sources', zorder=2, lw=0.5,ls = '-'
                        )
                        ax1.add_patch(circle)
            ...
            if variable_sources is not None and not self.input_yaml['HST_mode']:
                ...
                if len(variable_sources)>0:
                    ...
                    for x, y, otype,name in zip(variable_sources['x_pix'], variable_sources['y_pix'], variable_sources['OTYPE_opt'],variable_sources['MAIN_ID']):
                        
                        
                        # Draw "x" as two lines rotated 45 degrees (magenta for strong contrast)
                        ax1.plot([x - cross_len, x + cross_len], [y - cross_len, y + cross_len], color='#CC79A7', lw=0.5, zorder=2)
                        ax1.plot([x - cross_len, x + cross_len], [y + cross_len, y - cross_len], color='#CC79A7', lw=0.5, zorder=2)
                        
                        
                        if 'SN*' in otype: otype = name
                        # Annotate otype centered below the cross
                        ax1.annotate(
                            otype,
                            xy=(x, y),
                            xytext=(x, y+cross_len*2),  # offset down in display (pixels)
...
# =============================================================================
    @staticmethod
    def plot_lightcurve(output_file, snr_limit=3, beta_limit=0.5, fwhm=3, method='PSF', reference_epoch=0,
        ...
        for b in bandlist:
            ...
            if method == 'PSF' and 'SNR_PSF' in data_band.columns:
            ...
            elif method == 'PSF' and 'flux_PSF' in data_band.columns and 'flux_PSF_err' in data_band.columns:
                err_psf = np.asarray(data_band['flux_PSF_err'], dtype=float)
                snr = np.divide(data_band['flux_PSF'], err_psf, out=np.full(len(data_band), np.nan),
                ...
                snr = np.divide(data_band[band], err_vals, out=np.zeros(len(data_band)), where=err_vals > 0)

            # Detection: SNR >= snr_limit and (optionally) beta > beta_limit
            beta_ok = (data_band['beta'] > beta_limit) if 'beta' in data_band.columns else np.ones(len(data_band), dtype=bool)
            ...
            if not detects.empty:
                ax1.errorbar(
                    detects.mjd - reference_epoch, detects[band], yerr=detects[err_col],
                    c=cols.get(b, 'k'), ls='', capsize=2, marker=marker, label=b,
                )
            if show_limits and not nondetects.empty:
                # Upper limits: plot at limiting magnitude (fainter than this = non-detection)
                y_lim = nondetects['lmag'] if 'lmag' in nondetects.columns and np.any(np.isfinite(nondetects['lmag'])) else nondetects[band]
                ax1.errorbar(
                    nondetects.mjd - reference_epoch, y_lim,
                    c=cols.get(b, 'k'), ls='', marker='v', markersize=5, markerfacecolor='none',
                    markeredgewidth=0.5, alpha=0.85, zorder=0,
                )
