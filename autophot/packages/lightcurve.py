...
# =============================================================================
    ...
    fig = plt.figure('lightcurve_AUTOPHOT',figsize = set_size(504.0,aspect = 0.25))
    ...
    for _x,_y, _y_e ,_y_color, _y_color_e,_y_l,_s, _c, _m ,_f,_fil,_fwhm in zip(mjd, mag, mag_e,mag_color,mag_color_e,lmag, SNR, color, marker, fname,filters,fwhm_source):
            ...
            for axes in subp:
                ...
                if _y_e >= max_error_lim:
                    continue

                if  _y_l < _y or np.isnan(_y) or check_fwhm_iter:
    ...
    try:
        plt.savefig(os.path.join(syntax['write_dir'],'lightcurve.pdf'),
                    # box_extra_artists=([first_legend,second_legend]),
                            bbox_inches='tight')
    except:
        print('Saving lightcurve to: %s'% syntax['fits_dir'])
