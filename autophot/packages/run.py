...
def run_autophot(syntax):
    ...
    if syntax['fname'] != None:
    ...
    elif not syntax['prepare_templates']:
        flist = []
        ...
        template_loc = os.path.join(syntax['fits_dir'],'templates')
    ...
    if syntax['restart'] and not syntax['prepare_templates']:
        ...
        for i in flist:
            ...
            clean_path = os.path.join(path, file).replace('_APT','').replace(' ','_').replace('_'+syntax['outdir_name'],'')

            flist_before.append(clean_path)
        ...
        for root, dirs, files in os.walk((syntax['fits_dir']+'_'+syntax['outdir_name']).replace(' ','')):
            for fname in files:
                if '_APT.f' in fname:
                # if fname.endswith(("_APT.fits",'_APT.fit','_APT.fts','_APT.fz')):
...
# =============================================================================
    ...
    if syntax['method'] == 'sp':
        ...
        if not syntax['prepare_templates']:
            ...
            # failurefile = os.path.join(syntax['write_dir'],)
