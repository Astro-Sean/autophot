...
class FitsInfo:
    ...
    def __init__(self, input_yaml, flist=None, template_files=False):
        ...
        self.available_filters = list(AutophotYaml(filters_path).load().get('W_eff', {}).keys())

        self.logger.info(f"Initialized: {len(self.flist)} files")
    
    def ask_question(self, question, default_answer='n', expect_answer_type=str, options=None, ignore_word='skip'):
        """
    ...
    def find_similar_keywords(self, keywords, search_term, cutoff=0.6):
        ...
        """
        matches = get_close_matches(search_term.upper(), 
                                  [k.upper() for k in keywords], 
                                  n=10, cutoff=cutoff)
        return sorted([keywords[[k.upper() for k in keywords].index(m)] for m in matches])
    ...
    def ask_for_keyword(self, keyword, header, fname='', expected_units=None):
        ...
        if candidates:
            ...
            for i, k in kw_dict.items():
                val = str(header[k])[:20] + '...' if len(str(header[k])) > 20 else str(header[k])
                print(f"  [{i}] {k:<15} | {val}")
    ...
    def check(self):
        ...
        for fname in tqdm(self.flist):
            ...
            tele_key = next((k for k in KEYWORD_ALIASES['TELESCOP'] if k in header), None)
            inst_key = next((k for k in KEYWORD_ALIASES['INSTRUME'] if k in header), None)
        ...
        if incorrect_files and not self.template_files:
            ...
            with open(outfile, 'w') as f:
                for fname, has_t, has_i in incorrect_files:
                    missing = []
                    if not has_t: missing.append('TELESCOP')
                    if not has_i: missing.append('INSTRUME')
                    f.write(f"{fname}: missing {', '.join(missing)}\n")
            self.logger.info("%d files with missing TELESCOP/INSTRUME headers -> %s",
                        len(incorrect_files), outfile)
        
        # PHASE 2: BUILD TELESCOPE DATABASE
        db = self._load_db()
        
        self.logger.info("Telescopes discovered in headers: %s", ', '.join(tele_dict.keys()))
            ...
            for inst_key, insts in inst_groups.items():
                for inst_name in insts:
                    ...
                    if not entry:  # New instrument
                        entry['Name'] = self.ask_question(f"Label '{tele}+{inst_name}'", f"{tele}+{inst_name}")
                        ...
                        for fname in correct_files:
                            h = get_header(fname)
                            tele_key_hdr = next((k for k in KEYWORD_ALIASES['TELESCOP'] if k in h), None)
                            inst_key_hdr = next((k for k in KEYWORD_ALIASES['INSTRUME'] if k in h), None)
                            if not tele_key_hdr or not inst_key_hdr:
                                continue
                            if h[tele_key_hdr].strip() == tele and h[inst_key_hdr].strip() == inst_name:
                                sample_file = fname
                                break
                    ...
                    if correct_files:
                        ...
                        for opt, info in OPTIONAL_KEYWORDS.items():
                            ...
                            if label not in entry:
                                entry[label] = self.ask_for_keyword(opt, h, os.path.basename(correct_files[0]), info['units'])
        ...
        for fname in tqdm(correct_files):
            ...
            if tele not in db or inst_key not in db[tele] or inst not in db[tele][inst_key]:
                continue
    ...
    def _ask_filter_mapping(self, header_value):
        ...
        """
        opts = sorted(self.available_filters, key=lambda x: (x not in "ugriz", x.lower()))
        opts_str = ", ".join(opts)
        default = "no_filter"
        prompt = (
            "\n"
    ...
    def _find_filter_key(self, header, entry):
        ...
        next_n = max([int(re.search(r'filter_key_(\d+)', k).group(1)) for k in entry if 'filter_key_' in k] + [0]) + 1
