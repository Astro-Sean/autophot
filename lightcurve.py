...
BAND_WAVELENGTHS = {
    ...
    "U": 3600, "B": 4400, "V": 5500, "R": 6580, "I": 8060,
    "J": 12350, "H": 16620, "K": 21590,
}

# Hand-tuned plotting colours: wavelength-inspired for optical, distinct for IR.
BAND_COLORS = {
    "u": "#4B0082", "g": "#00BFFF", "r": "#FF4500", "i": "#B22222", "z": "#800000",
    "U": "#7B2FBE", "B": "#1E90FF", "V": "#228B22", "R": "#DC143C", "I": "#8B0000",
    "J": "#A0522D", "H": "#DAA520", "K": "#556B2F",
}

# Colour indices -> plotting colour (distinct, hue between constituent bands).
COLOR_INDEX_COLORS = {
    "u-g": "#6A0DAD", "g-r": "#20B2AA", "r-i": "#E55B3C", "i-z": "#A52A2A",
    "g-i": "#4682B4", "r-z": "#CD5C5C", "u-r": "#8A2BE2", "u-i": "#9370DB",
    "u-z": "#7B68EE", "U-B": "#9932CC", "B-V": "#3CB371", "V-R": "#FFD700",
    "V-I": "#FF8C00", "R-I": "#FF6347", "B-R": "#00CED1", "B-I": "#1E90FF",
    "V-J": "#BDB76B", "V-K": "#6B8E23", "J-H": "#D2691E", "H-K": "#8B8000",
    "J-K": "#BC8F8F", "i-J": "#C71585", "i-H": "#DB7093", "z-J": "#8B4513",
}
...
def _normalize_photometry_columns(df):
    ...
    for c in df.columns:
        ...
        if (
            ...
            norm_cols.append(s)
        elif s.lower().startswith("zp_") and len(s) >= 6 and s[3].isalpha() and s[4] == "_":
            norm_cols.append(s)
        else:
...
def _resolve_band_triplet(cols, band: str, method: str):
    ...
    cand = [
        ...
        (f"{band}_{method}".lower(), f"{band}_{method}_err".lower(), f"zp_{band}_{method}".lower()),
    ]
    for trip in cand:
...
def plot_lightcurve(
    ...
    if size is None:
        if single_plot:
            ...
            figsize = (set_size(540)[0], set_size(505, aspect=1)[1] * len(bands_in_data))
    else:
        figsize = size
    ...
    mid_idx = (len(bands_in_data) - 1) // 2  # reference band (offset 0) is the middle one
    ...
    for idx, band in enumerate(bands_in_data):
        ...
        if use_SNR_limit:
            ...
            elif method == 'PSF' and 'flux_psf' in df.columns and 'flux_psf_err' in df.columns:
        ...
        beta_ok = (
            (pd.to_numeric(df['beta'], errors='coerce').to_numpy(dtype=float, copy=False) > float(beta_limit))
        ...
        if use_SNR_limit:
            ...
            detected = (
                np.isfinite(mag.to_numpy(dtype=float, copy=False))
                & np.isfinite(mag_err.to_numpy(dtype=float, copy=False))
                & (mag.to_numpy(dtype=float, copy=False) < lmag.to_numpy(dtype=float, copy=False))
                & beta_ok
            )
        ...
        c = cols.get(band, 'k')
        if offset != 0:
            leg_label = band if band_offset == 0 else (f'{band}{band_offset:+.0f}' if band_offset == int(band_offset) else f'{band}{band_offset:+.1f}')
        else:
            leg_label = band
    ...
    xlabel = f'Phase (days since {reference_epoch})' if reference_epoch else 'Time [MJD]'
    ...
    if show_limits and has_limits_plotted and 'Upper limit' not in labels:
        limit_handle = Line2D(
            [0], [0], color='gray', marker='v', markersize=5,
            markerfacecolor='none', markeredgecolor='gray', markeredgewidth=0.5, ls='', label='Upper limit'
        )
    ...
    if plot_color and color_ax is not None and 'filter' in data.columns:
        ...
        for (b1, b2) in _COLOR_PAIRS:
            ...
            col1, err1, zp1 = t1
            col2, err2, zp2 = t2
            fcol = data['filter'].astype(str).str.lower() if 'filter' in data.columns else None
            d1 = data[(fcol == b1.lower()) & np.isfinite(data[zp1])].copy() if fcol is not None else pd.DataFrame()
            d2 = data[(fcol == b2.lower()) & np.isfinite(data[zp2])].copy() if fcol is not None else pd.DataFrame()
            d1['mag'] = d1[col1]
            d1['err'] = d1[err1]
            d2['mag'] = d2[col2]
            d2['err'] = d2[err2]
            ...
            if use_SNR_limit:
                if method == 'PSF' and 'snr_psf' in d1.columns:
                ...
                elif method == 'PSF' and 'flux_psf' in d1.columns and 'flux_psf_err' in d1.columns:
                    snr1 = np.divide(d1['flux_psf'], d1['flux_psf_err'], out=np.full(len(d1), np.nan), where=(np.asarray(d1['flux_psf_err']) > 0) & np.isfinite(d1['flux_psf_err']))
                    snr2 = np.divide(d2['flux_psf'], d2['flux_psf_err'], out=np.full(len(d2), np.nan), where=(np.asarray(d2['flux_psf_err']) > 0) & np.isfinite(d2['flux_psf_err']))
                    ...
                    snr1 = np.divide(d1['mag'], d1['err'], out=np.zeros_like(d1['mag']), where=d1['err'] > 0)
                    snr2 = np.divide(d2['mag'], d2['err'], out=np.zeros_like(d2['mag']), where=d2['err'] > 0)
            else:
                snr1 = np.divide(d1['mag'], d1['err'], out=np.zeros_like(d1['mag']), where=d1['err'] > 0)
                snr2 = np.divide(d2['mag'], d2['err'], out=np.zeros_like(d2['mag']), where=d2['err'] > 0)
            beta_ok1 = (d1['beta'] > beta_limit) if 'beta' in d1.columns else np.ones(len(d1), dtype=bool)
            beta_ok2 = (d2['beta'] > beta_limit) if 'beta' in d2.columns else np.ones(len(d2), dtype=bool)
            if use_SNR_limit:
                d1['det'] = np.isfinite(d1['mag']) & np.isfinite(d1['err']) & np.isfinite(snr1) & (snr1 >= snr_limit) & beta_ok1
                d2['det'] = np.isfinite(d2['mag']) & np.isfinite(d2['err']) & np.isfinite(snr2) & (snr2 >= snr_limit) & beta_ok2
            else:
                d1['det'] = np.isfinite(d1['mag']) & np.isfinite(d1['err']) & (d1['mag'] < d1['lmag']) & beta_ok1
                d2['det'] = np.isfinite(d2['mag']) & np.isfinite(d2['err']) & (d2['mag'] < d2['lmag']) & beta_ok2
            ...
            # Color = b1 - b2. Limit direction: b1 det + b2 limit -> true color <= val (upper limit, v);
            # b1 limit + b2 det -> true color >= val (lower limit, ^).
            ...
            if phase_pts:
                ...
                color_ax.errorbar(
                    phase_arr, color_arr, yerr=err_arr,
                    color=c, ecolor=c, marker='s', markersize=4, ls='', capsize=1.5,
                    markeredgecolor='black', markeredgewidth=0.5,
                    label=label, zorder=2,
                )
            if phase_ul:
                color_ax.errorbar(
                    np.array(phase_ul), np.array(color_ul),
                    color=c, markeredgecolor=c, markerfacecolor='none', markeredgewidth=0.5,
                    marker='v', markersize=4, ls='', zorder=2,
                )
            if phase_ll:
                color_ax.errorbar(
                    np.array(phase_ll), np.array(color_ll),
                    color=c, markeredgecolor=c, markerfacecolor='none', markeredgewidth=0.5,
                    marker='^', markersize=4, ls='', zorder=2,
                )
...
def generate_photometry_table(
    ...
    if "filter" in complete_data.columns:
        ...
        bands = sorted(bands, key=lambda b: BAND_WAVELENGTHS.get(b, BAND_WAVELENGTHS.get(b.lower(), 1e12)))
    else:
    ...
    for band in bands:
        ...
        if use_SNR_limit:
            if method == 'PSF' and 'snr_psf' in data.columns:
            ...
            elif method == 'PSF' and 'flux_psf' in data.columns and 'flux_psf_err' in data.columns:
                snr = np.divide(data['flux_psf'], data['flux_psf_err'], out=np.full(len(data), np.nan), where=(np.asarray(data['flux_psf_err']) > 0) & np.isfinite(data['flux_psf_err']))
                ...
                snr = np.divide(data[col], data[err_col], out=np.zeros_like(data[col]), where=data[err_col] > 0)
        else:
            snr = np.divide(data[col], data[err_col], out=np.zeros_like(data[col]), where=data[err_col] > 0)
        beta_ok = (data['beta'] > beta_limit) if 'beta' in data.columns else np.ones(len(data), dtype=bool)
        if use_SNR_limit:
            detected = np.isfinite(data[col]) & np.isfinite(data[err_col]) & np.isfinite(snr) & (snr >= snr_limit) & beta_ok
        else:
            detected = np.isfinite(data[col]) & np.isfinite(data[err_col]) & (data[col] < data['lmag']) & beta_ok
        ...
        if not detects.empty:
            detects = detects.assign(
                Filter=band, Limit='-', MJD=detects['mjd'].round(3),
                Date=Time(detects['mjd'], format='mjd').iso,
                Mag=detects[col].round(3), Error=detects[err_col].round(3)
            )[["MJD", "Date", "Mag", "Error", "Filter", "Limit"]]
            phot_table.append(detects)

        if not nondetects.empty:
            nondetects = nondetects.assign(
                Filter=band, MJD=nondetects['mjd'].round(3),
                Date=Time(nondetects['mjd'], format='mjd').iso,
                Mag='-', Error='-', Limit=nondetects['lmag'].round(3)
            )[["MJD", "Date", "Mag", "Error", "Filter", "Limit"]]
            phot_table.append(nondetects)

    if not phot_table:
        out_phot = pd.DataFrame(columns=["MJD", "Date", "Mag", "Error", "Filter", "Limit"])
    else:
    ...
    if include_color_table and 'filter' in complete_data.columns and len(complete_data) > 1:
        color_rows = []
        for (b1, b2) in _COLOR_PAIRS:
            ...
            d1 = complete_data[(fcol == b1.lower()) & np.isfinite(complete_data[zp1])].copy()
            d2 = complete_data[(fcol == b2.lower()) & np.isfinite(complete_data[zp2])].copy()
            if d1.empty or d2.empty:
                continue
            d1['mag'] = d1[col1]
            d1['err'] = d1[err1]
            ...
            for _, row in d1.iterrows():
                ...
                if row['det'] and det2[j]:
                    ...
                    color_rows.append({
                        'MJD': round(mjd_mid, 3), 'Date': date_str, 'Phase': round(phase, 3) if np.isfinite(phase) else '-',
                        'Color': color_name, 'Value': round(val, 3), 'Error': round(err_val, 3), 'Limit': '-',
                    })
                elif row['det'] and not det2[j]:
                    val = row['mag'] - lmag2[j]
                    color_rows.append({
                        'MJD': round(mjd_mid, 3), 'Date': date_str, 'Phase': round(phase, 3) if np.isfinite(phase) else '-',
                        'Color': color_name, 'Value': round(val, 3), 'Error': '-', 'Limit': 'upper',
                    })
                elif not row['det'] and det2[j]:
                    val = row['lmag'] - mag2[j]
                    color_rows.append({
                        'MJD': round(mjd_mid, 3), 'Date': date_str, 'Phase': round(phase, 3) if np.isfinite(phase) else '-',
                        'Color': color_name, 'Value': round(val, 3), 'Error': '-', 'Limit': 'lower',
                    })
        if color_rows:
            color_df = pd.DataFrame(color_rows)
            color_df.sort_values(['MJD', 'Color'], inplace=True)
            color_table_path = os.path.join(save_path, f'lightcurve_{method}_colors.dat')
...
def check_detection_plots(output_file, method='PSF'):
    ...
    for _, row in data.iterrows():
        try:
            ...
            band = row['filter']
            src_file = files[0]
            dest_file = os.path.join(save_path, f'{band}band_{date}_{os.path.basename(src_file)}')
