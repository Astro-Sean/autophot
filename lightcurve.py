...
def plot_lightcurve(
    ...
    for idx, band in enumerate(bands_in_data):
        ...
        # Initialize plot_mag and plot_err for all detections to avoid KeyError
        # (will be overwritten for inverted-only detections below)
        ...
        if not all_detects.empty:
            # Check which rows used inverted fit
            if "_inverted_fit" in all_detects.columns:
                inv_fit_values = all_detects["_inverted_fit"]
                if inv_fit_values.dtype == object:
