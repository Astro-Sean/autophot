...
def cross_match_sources(given_catalog, variable_catalog, match_radius_pix=5):
    ...
    for i, (xg, yg) in enumerate(zip(x_given, y_given)):
        ...
        if np.any(within_radius):
            matched_var_idx = np.where(within_radius)[0][0]  # First matched variable source index
            keep_mask[i] = False
            removed_indices.append(i)
            otype = variable_catalog.iloc[matched_var_idx]['OTYPE_opt']
            logger.debug(f"Removing source at index {i} (x={xg:.2f}, y={yg:.2f}) due to match with variable source [{otype}]")
...
# =============================================================================
    ...
    def gaia_synthetic_photometry(self,ra, dec, radius=0.1, max_sources=5000):
        ...
        try:
            logger.info("Querying Gaia DR3 (synthetic photometry, max %d sources)...", max_sources)
            ...
            logger.info("Downloading Gaia XP spectra and synthetic photometry with GaiaXPy...")
            try:
            ...
            for band in ['u', 'g', 'r', 'i', 'z']:
                ...
                merged.loc[valid, m_err_col] = factor * merged.loc[valid, e_col] / merged.loc[valid, f_col]
            ...
            logger.error("Gaia DR3 synthetic photometry query failed: %s", exc, exc_info=True)
            return pd.DataFrame()
...
# =============================================================================
    ...
    def query_legacy_survey(self, ra, dec, radius=0.1):
        ...
        try:
            logging.info(f'Fetching Legacy Survey Dr10 catalog over {radius:.1f} field-of-view centered at ra = {ra:.1f} dec = {dec:.1f}')
            logging.info(query)
...
# =============================================================================
    ...
    def fetch_refcat2_field(self, ra, dec, credentials, nsources=1000, sr=0.5):
        ...
        try:
            ...
            logger.info(f'Fetching ATLAS-RefCat2 catalog from MAST over {sr:.3f} deg field-of-view centered at ra = {ra:.1f} dec = {dec:.1f}')

            # Define the columns to be retrieved from the database
            table = ['RA', 'Dec', 'g', 'dg', 'r', 'dr', 'i', 'di', 'z', 'dz', 'J', 'dJ', 'H', 'dH', 'K', 'dK']
...
# =============================================================================
    ...
    def download(self, target_coords, catalogName, radius=10, target_name=None,
            ...
            if catalogName == 'custom':
                ...
                if catalogName == 'tic':
                    ...
                    selected_cols = [
                        "ID", "ra", "dec",
                        "rmag", "e_rmag",
                        "gmag", "e_gmag", "imag", "e_imag", "zmag", "e_zmag", "umag", "e_umag",
                        "Bmag", "e_Bmag", "Vmag", "e_Vmag",
                        "Jmag", "e_Jmag", "Hmag", "e_Hmag", "Kmag", "e_Kmag",
                        "GAIAmag", "e_GAIAmag", "Tmag", "e_Tmag"
                    ]

                    # Ensure all selected columns exist in the result table
                    available_cols = [col for col in selected_cols if col in result.colnames]
                    ...
                    cfg_radius = float(self.input_yaml.get("catalog", {}).get("gaia_xp_radius_deg", max_gaia_deg))
                    ...
                    selectedCatalog.to_csv(f"{fname}.csv", index=False, na_rep=np.nan)
                    shutil.move(os.path.join(os.getcwd(), f"{fname}.csv"),
                                os.path.join(target_dir,  f"{fname}.csv"))
                    ...
                    params = {'RA': target_coords.ra.degree, 'DEC': target_coords.dec.degree, 'SR': radius_deg, 'RESPONSEFORMAT': 'VOTABLE'}
                    ...
                    logger.info(f'Downloading reference sources from {catalogName.upper()}')
                    selectedCatalog = Catalogs.query_region(target_coords, radius=0.1*u.deg, catalog='Panstarrs').to_pandas()
...
# =============================================================================
    def clean(
        ...
        try:
            ...
            catalog_keywords = autophot_yaml(
                os.path.join(filepath, 'databases', catalog_autophot_input_yml),
                catalogName
            ).load()

            max_distance = self.input_yaml['catalog'].get('max_distance', 10)  # arcminutes
            ...
            if ra_key not in selectedCatalog or dec_key not in selectedCatalog:
                raise KeyError(f"Required RA/DEC columns '{ra_key}', '{dec_key}' not found in catalog.")
            ...
            if image_wcs:
                try:
                    ...
                    logger.warning(f"Failed to convert RA/DEC to pixel coordinates: {e}. Skipping pixel coordinate conversion.")
            ...
            baseDatabase = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'databases')
            ...
            for filter_x in availableFilters['default_dmag'].keys():
                for col in [filter_x, f"{filter_x}_err"]:
                    if filter_x in catalog_keywords and catalog_keywords.get(col) in selectedCatalog:
                        outputCatalog[col] = selectedCatalog[catalog_keywords[col]].values

            # --- Early return if only updating names ---
            if update_names_only:
                logger.info(f'{len(outputCatalog)} sources in output catalog (names only)')
                return outputCatalog

            # --- Determine filters to use ---
            usefilter = usefilter or [self.input_yaml['imageFilter']]
            magCutoff = [magCutoff] if isinstance(magCutoff, (int, float)) else magCutoff
            ...
            if not full_clean:
                ...
                if tooFaint.any():
                    logger.info(f'Removing {tooFaint.sum()} sources that are fainter than the cutoff')
...
# =============================================================================
    ...
    def recenter(self, selectedCatalog, image, boxsize=None, error=None):
        ...
        try:
            ...
            logger.info(
                border_msg(f"Recentering {num_sources} source{'s' if num_sources > 1 else ''}")
            )

            required_columns = {"x_pix", "y_pix"}
            if not required_columns.issubset(selectedCatalog.columns):
                logger.error("Selected catalog missing required columns: 'x_pix', 'y_pix'")
                return selectedCatalog
            ...
            valid_sources = self._check_valid_cutouts(image, old_x, old_y, boxsize, nan_mask)
            ...
            if use_error_weighted:
                ...
                if err_arr.shape != image.shape:
                    if np.isscalar(err_arr) or err_arr.size == 1:
                        ...
                        err_arr = np.broadcast_to(np.nanmedian(err_arr), image.shape).copy()
            ...
            if use_error_weighted:
                ...
                centroid_func = (
                    centroid_2dg_with_error if undersampled
                    else centroid_com_with_error
                )
                logger.info(
                ...
                try:
                    x_valid, y_valid, x_err_valid, y_err_valid = _centroid_sources_with_error_impl(
                        image, old_x_valid, old_y_valid,
                        box_size=boxsize, mask=nan_mask, error=err_arr,
                    ...
                    logger.warning(f"Error-weighted centroiding failed: {e}; falling back to unweighted")
                    use_error_weighted = False
            if not use_error_weighted:
                ...
                try:
                    x_valid, y_valid = centroid_sources(
                        image, old_x_valid, old_y_valid,
                        box_size=boxsize, centroid_func=centroid_func, mask=nan_mask,
                    )
            ...
            valid = np.isfinite(x) & np.isfinite(y) & np.isfinite(old_x) & np.isfinite(old_y)
            if valid.any():
                average_offset = np.nanmedian(pix_dist(x[valid], old_x[valid], y[valid], old_y[valid]))
                logger.info(f"Median pixel correction: {average_offset:.1f} px")
            else:
                logger.warning("No valid sources to compute median offset.")
    
            # Remove NaNs
            selectedCatalog = selectedCatalog.loc[selectedCatalog[["x_pix", "y_pix"]].notna().all(axis=1)]
            logger.info(
    ...
    def build_complete_catalog(self, target_coords, target_name=None,
        ...
        for catalogName in catalog_list:
            ...
            for index, entry in catalog_i.iterrows():
                ...
                existing_source = self.find_source(ra, dec, output_catalog, tolerance=tolerance_arcsec)
...
# =============================================================================
    ...
    def check_saturation_range(self, catalog, threshold=5):
        ...
        try:
            ...
            logger.info(border_msg(f"Checking saturation level using {len(catalog)} sources"))
            ...
            missing_columns = [col for col in required_columns if col not in catalog.columns]
            if missing_columns:
                raise KeyError(f"Missing required columns in catalog: {missing_columns}")

            # Sigma-clip sky background
            if 'sky' in catalog.columns:
                noise_mask = sigma_clip(np.abs(catalog['sky'].values), sigma=5, maxiters=10)
                if np.any(noise_mask.mask):
                    logger.info(f"Masking {np.sum(noise_mask.mask)} sources with irregular backgrounds")
                    catalog = catalog[~noise_mask.mask]

            # Threshold filtering
            if 'threshold' in catalog.columns:
                threshold_cut = catalog['threshold'] < threshold
                logger.info(f"Removing {np.sum(threshold_cut)} sources with threshold < {threshold}")
            ...
            class ConstrainedSlopeRegressor(BaseEstimator, RegressorMixin):
                def __init__(self, slope_constraint=1.0, slope_tolerance=1e-3, sample_weight=None):
                ...
                def fit(self, X, y):
                    ...
                    def loss(params):
                        ...
                        if weights is not None:
                            ...
                            mse = np.mean(resid**2)
                        slope_penalty = 100 * max(0, abs(slope - self.slope_constraint) - self.slope_tolerance)
            ...
            if len(inst_mag_linear) > 1:
                base_estimator = ConstrainedSlopeRegressor(slope_constraint=1.0, slope_tolerance=0)
                ...
                intercept_error = residual_std * np.sqrt(1/n_points + x_mean**2/((n_points-1)*x_var))
                ...
                logger.info(f"Constrained fit results:  Zeropoint = {intercept:.3f} +/- {intercept_error:.3f}")
                ...
                if len(inlier_flux) > 0:
                    ...
                    logger.info(f"Linearity range (0.5-95% flux percentiles): {min_flux:.1f} - {max_flux:.1f}")
                else:
            ...
            ax1.errorbar(
                inst_mag, catalog_mag, yerr=catalog_mag_err, fmt='o',
                alpha=0.35, color='gray', markersize=2.2, capsize=0, elinewidth=0.4,
                linestyle='None', label=f"All Sources [{len(inst_mag)}]"
            )
            ...
            if fit_line:
                ...
                ax1.errorbar(
                    ...
                    xerr=inst_mag_err_linear[outlier_mask], fmt='o', color='red',
                    markersize=2.2, capsize=0, elinewidth=0.4, linestyle='None',
                    label=f"Outliers [{np.sum(outlier_mask)}]"
                )
                ax1.errorbar(
                    inst_mag_linear[inlier_mask], catalog_mag_linear[inlier_mask],
                    yerr=catalog_mag_err_linear[inlier_mask],
                    xerr=inst_mag_err_linear[inlier_mask], fmt='o', color='blue',
                    markersize=2.2, capsize=0, elinewidth=0.4, linestyle='None',
                    label=f"Inliers [{np.sum(inlier_mask)}]"
                )
                x_range = np.linspace(inst_mag.min(), inst_mag.max(), 100)
                ax1.plot(x_range, fit_line(x_range), color='black', linestyle='--',
                ...
                if len(inlier_flux) > 0:
                    min_inst_mag = -2.5 * np.log10(max_flux)  # Note: brighter objects have smaller magnitudes
                    max_inst_mag = -2.5 * np.log10(min_flux)
                    ax1.axvline(x=min_inst_mag, color='green', linestyle=':', alpha=0.7,
            ...
            exc_type, exc_obj, exc_tb = sys.exc_info()
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            logger.error(f"Error in build_psf: {exc_type} in {fname} at line {exc_tb.tb_lineno}: {str(e)}")
...
# =============================================================================
    ...
    def downsample_sources_by_position(
        ...
        logger.info(border_msg(f"Starting downsampling: {n_src} sources -> target {nmax} sources"))
        ...
        logger.info(f"Using {n_bins}x{n_bins} grid with ~{sources_per_bin} sources per bin")
            ...
            for j in range(n_bins):
                ...
                if len(bin_indices) > 0:
                    ...
                    if snr_col and snr_col in df.columns:
                        bin_indices = bin_indices[np.argsort(df[snr_col].iloc[bin_indices])[::-1]]
                        logger.debug(f"Bin ({i},{j}): {len(bin_indices)} sources, sorted by {snr_col}")
                    else:
        ...
        # Log bin statistics
        logger.info(f"Bin statistics: min={min(bin_stats)}, max={max(bin_stats)}, "
        ...
        if len(selected_indices) < nmax:
            logger.warning(f"Only {len(selected_indices)} sources selected from bins, "
            ...
            logger.info(f"Added {additional_count} additional sources from remaining pool")
...
# =============================================================================
    ...
    def check_starshape(self, image, catalog, scale=25, threshold=5,
                        fwhm_threshold=1.5,
                        roundness_threshold=0.2, sharpness_threshold=0.4):
        """
        ...
        if not required_columns.issubset(catalog.columns):
            raise ValueError(f"Catalog missing required columns: {required_columns - set(catalog.columns)}")
        ...
        def norm(array):
            ...
            if max_val == min_val:
                return np.zeros_like(array)  # Avoid division by zero; all elements are the same.
        ...
        for i, (x, y) in enumerate(zip(catalog['x_pix'], catalog['y_pix'])):
            ...
            try:
                cutout = Cutout2D(image, position=position, size=scale, mode='partial', fill_value=0)
                ...
                logger.warning(f"Star {i} at position {position} skipped due to error: {e}")
                for key in metrics:
        ...
        roundness_sigma_clip = sigma_clip(metrics['roundness'], sigma=threshold, masked=True, cenfunc=np.nanmedian, stdfunc=np.nanstd)
        sharpness_sigma_clip = sigma_clip(metrics['sharpness'], sigma=threshold, masked=True, cenfunc=np.nanmedian, stdfunc=np.nanstd)
        ...
        if len(radial_profiles) > 0:
            ...
            clipped = sigma_clip(radial_profiles, sigma=threshold, axis=0,
                                 masked=True,
        ...
        inliers = [valid_indices[i] for i, is_good in enumerate(combined_mask) if is_good]
        outliers = [valid_indices[i] for i, is_good in enumerate(combined_mask) if not is_good]

        # Log the results
        logger.info(f"Point sources: {len(inliers)} | Extended/rejected sources: {len(outliers)}")
        logger.info(f"Rejection breakdown - FWHM: {sum(~fwhm_mask)}, "
        ...
        if len(inliers) > 0:
            ...
            logger.warning("No sources remained after cleaning. Using original catalog.")
        ...
        if self.input_yaml.get('save_cutout_plot', True):
            ...
            for i in range(num_stars):
                ...
                ax.imshow(stars[i], origin='lower', cmap='viridis', norm=norm, interpolation='none')
                ax.text(0.98, 0.98, f'{i + 1}/{num_stars}', transform=ax.transAxes, va='top', ha='right', color='white')
                ax.set_xticks([])
                ax.set_yticks([])

            ax_right = fig.add_subplot(grid[:, -2:])
            radii = np.arange(radial_profiles.shape[1])
            accepted_profiles = [radial_profiles[valid_indices.index(i)] for i in inliers]
            ...
            ax_right.step(radii, median_profile, color='red', linewidth=0.5,
                          label='Median\nRadial\nProfile')
...
# =============================================================================
    ...
    def measure(self, selectedCatalog, image):
        ...
        logger.info('Instrumental magnitude of %d sources measured' % sum(~np.isnan(selectedCatalog[inst_col])))
